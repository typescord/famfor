...
export interface FormDataField extends Omit<FormDataFieldOptions, 'fetchReadStreamSize'> {
	value: FormDataValue;
	header: string;
}

// a very bad way to override parent methods signature...
interface FormDataMap extends Omit<Map<string, FormDataField[]>, 'forEach' | 'set'> {
	get(name: string): FormDataField[] | undefined;
	get(name: string): any;
	values(): IterableIterator<FormDataField>;
	values(): any;
	entries(): IterableIterator<[string, FormDataField[]]>;
	entries(): any;
}

declare const Map: {
	new (): FormDataMap;
	readonly prototype: FormDataMap;
};
...
export class FormData extends Map {
	...
	private set = undefined;
	private forEach = undefined;
	...
	private async *release(): AsyncGenerator<Buffer | string, any, undefined> {
		for (const [, fields] of super.entries()) {
	 ...
	 * and when `options.size` is undefined (`undefined`/`null`), the
	...
	public append(name: string, value: FormDataValue, options: FormDataFieldOptions = {}): void {
		...
		// @ts-expect-error ignoring this is the best solution.
		entry ? entry.push(field) : super.set(name, [field]);
	}
	...
	public get<T extends FormDataValue[] = FormDataValue[]>(name: string): T | undefined {
		return super.get(name)?.map(({ value }: FormDataField) => value) as T;
	}
	...
	public *entries<T extends FormDataValue = FormDataValue>(this: FormData): IterableIterator<[string, T]> {
		for (const [name, fields] of super.entries()) {
			for (const { value } of fields) {
